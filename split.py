...
def split_data(node_list, edge_list, node_predict, node_predict_round, edge_predict, edge_predict_round, edge_truth,
    ...
    """
    split data to 878 and 2000 randomly
    :param node_list:
    :param edge_list:
    :param node_predict:
    ...
    """
    random.seed(23)
    sample_index = random.sample(range(2878), 878)
    #
    node_str = ["l" + str(i) for i in node_list]
    edge_str = ["l" + str(i[0]) + "_" + "l" + str(i[1]) for i in edge_list]
    # for node
    for node_index in range(len(node_list)):
        node_predict_878 = []
        node_predict_878_round = []
        node_predict_2000 = []
        node_predict_2000_round = []
        for index in range(y_test.shape[0]):
            if index in sample_index:
                node_predict_878.append(node_predict[node_index][index])
                node_predict_878_round.append(node_predict_round[node_index][index])
            else:
                node_predict_2000.append(node_predict[node_index][index])
                node_predict_2000_round.append(node_predict_round[node_index][index])
        x_write(node_predict_878, "data/split_test/" + node_str[node_index] + "_predict_878.txt")
        x_write(node_predict_878_round, "data/split_test/" + node_str[node_index] + "_predict_878_round.txt")
        x_write(node_predict_2000, "data/split_test/" + node_str[node_index] + "_predict_2000.txt")
        x_write(node_predict_2000_round, "data/split_test/" + node_str[node_index] + "_predict_2000_round.txt")
    # for edge
    for edge_index in range(len(edge_list)):
        edge_predict_878 = []
        edge_predict_878_round = []
        edge_predict_2000 = []
        edge_predict_2000_round = []
        edge_truth_878 = []
        edge_truth_2000 = []
        for index in range(y_test.shape[0]):
            if index in sample_index:
                edge_predict_878.append(edge_predict[edge_index][index])
                edge_predict_878_round.append(edge_predict_round[edge_index][index])
                edge_truth_878.append(edge_truth[edge_index][index])
            else:
                edge_predict_2000.append(edge_predict[edge_index][index])
                edge_predict_2000_round.append(edge_predict_round[edge_index][index])
                edge_truth_2000.append(edge_truth[edge_index][index])
        x_write(edge_predict_878, "data/split_test/" + edge_str[edge_index] + "_predict_878.txt")
        x_write(edge_predict_878_round, "data/split_test/" + edge_str[edge_index] + "_predict_878_round.txt")
        x_write(edge_predict_2000, "data/split_test/" + edge_str[edge_index] + "_predict_2000.txt")
        x_write(edge_predict_2000_round, "data/split_test/" + edge_str[edge_index] + "_predict_2000_round.txt")
        x_write(edge_truth_878, "data/split_test/" + edge_str[edge_index] + "_truth_878.txt")
        x_write(edge_truth_2000, "data/split_test/" + edge_str[edge_index] + "_truth_2000.txt")
    # split y_test, original_predict, original_predict_round
    y_test_878 = []
    original_predict_878 = []
    original_predict_878_round = []
    y_test_2000 = []
    original_predict_2000 = []
    original_predict_2000_round = []
    for index in range(y_test.shape[0]):
        if index in sample_index:
            y_test_878.append(y_test[index])
            original_predict_878.append(original_predict[index])
            original_predict_878_round.append(original_predict_round[index])
        else:
            y_test_2000.append(y_test[index])
            original_predict_2000.append(original_predict[index])
            original_predict_2000_round.append(original_predict_round[index])
    y_write(y_test_878, "data/split_test/y_test_878.txt")
    y_write(y_test_2000, "data/split_test/y_test_2000.txt")
    y_write_float(original_predict_878, "data/store/original/original_predict_878.txt")
    y_write_float(original_predict_2000, "data/store/original/original_predict_2000.txt")
    y_write(original_predict_878_round, "data/store/original/original_predict_878_round.txt")
    y_write(original_predict_2000_round, "data/store/original/original_predict_2000_round.txt")
