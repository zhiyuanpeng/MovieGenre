...
with open("data/store/original/original_result.txt", "a+") as r:
    ...
    for i in range(y_test.shape[1]):
        ...
        if con_mat[0, 0] == 0.0:
            ...
            r.write("node l" + str(i) + " recall of 0 is " + str(con_mat[0, 0] / (con_mat[0, 0] + con_mat[1, 0])) + "\n")
        r.write("node l" + str(i) + " precision of 1 is " + str(con_mat_norm[1, 1]) + "\n")
        if con_mat[1, 1] == 0.0:
            r.write("node l" + str(i) + " recall of 1 is " + str(0.0) + "\n")
        else:
            r.write("node l" + str(i) + " recall of 1 is " + str(con_mat[1, 1] / (con_mat[0, 1] + con_mat[1, 1])) + "\n")
...
plt.legend()
plt.savefig("data/img/original_grad_90_size128.png")
plt.show()
